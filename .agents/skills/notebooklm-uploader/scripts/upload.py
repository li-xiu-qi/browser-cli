...
class ZLibraryAutoUploader:
    ...
    def __init__(self):
        ...
        self.downloads_dir = self.project_root / "resources" / "downloads" / "books"
        self.temp_dir = self.project_root / ".agents" / "skills" / "notebooklm-uploader" / "temp"
        self.config_dir = self.project_root / ".agents" / "tools" / "zlibrary" / "config"
        # 统一使用共享的浏览器数据目录
        self.user_data_dir = self.project_root / ".agents" / "browser_user_data"
        # 使用新的 zlibrary 工具目录
        self.config_dir = self.project_root / ".agents" / "tools" / "zlibrary" / "config"
        self.config_file = self.config_dir / "zl_tokens.json"
    ...
    async def login_to_zlibrary(self, page):
        ...
        if not credentials:
            ...
            print(f"💡 请先运行: python scripts/login.py")
            print(f"   位置: {Path(__file__).parent / 'login.py'}")
            return False

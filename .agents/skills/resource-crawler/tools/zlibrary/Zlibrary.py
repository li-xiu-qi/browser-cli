...
class Zlibrary:
    ...
    def __getBookFile(self, bookid: [int, str], hashid: str) -> [(str, bytes), None]:
        ...
        if "file" not in response:
            print(f"Error: 'file' not in response: {response}")
            return None
        
        file_info = response["file"]
        
        # 使用 name 或 description 作为文件名，兜底使用 "unknown"
        filename = file_info.get("name") or file_info.get("description") or "unknown"
        ...
        if not ddl:
            print(f"Error: 'downloadLink' not found in file info")
            return None
